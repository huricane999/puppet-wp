  ...
  case $ensure {
    enabled: {
      if $networkwide {
        ...
        $check = "/usr/bin/wp theme status ${theme_name} | /bin/grep -q Status:\\ Active"
      }
    }
    disabled: {
      if $networkwide {
        ...
        $check = "/usr/bin/wp theme status ${theme_name} | /bin/grep -q Status:\\ Inactive"
      }
    }
    /^(absent|uninstalled)$/: {
      # lint:ignore:140chars
      $command = "delete ${theme_name}"
      $only_if = "/bin/bash -c \"[[ `/usr/bin/wp theme list | /bin/grep ${theme_name} | /bin/awk '{print \\\$5}'` == 'no' ]]\""

      wp::command { "${location} disable theme ${theme_name}":
        location => $location,
        command  => "theme disable ${theme_name}",
        user     => $user,
        onlyif   => [
          "/bin/bash -c \"/usr/bin/wp theme list | /bin/grep -q ${theme_name}\"",
          "/bin/bash -c \"/usr/bin/wp theme list | /bin/grep ${theme_name} | /bin/awk '{print \\\$5}' | /bin/grep -q site\"",
        ],
        tag      => 'theme-uninstalled',
      }
      ->exec { "${location} deactivate theme ${theme_name}":
        command => "/bin/bash -c \"/usr/bin/wp theme activate `/usr/bin/wp theme list | /bin/grep -v ${theme_name} | /bin/grep -m1 -e \"network\\|site\" | /bin/awk '{print \$1}'`\"",
        cwd     => $location,
        user    => $user,
        onlyif  => [
          "/bin/bash -c \"/usr/bin/wp theme list | /bin/grep -q ${theme_name}\"",
          "/bin/bash -c \"[[ `/usr/bin/wp theme list | /bin/grep ${theme_name} | /bin/awk '{print \\\$2}'` == 'active' ]]\"",
        ],
        tag     => 'theme-uninstalled',
      }
      if $networkwide {
        wp::command { "${location} network disable theme ${theme_name}":
          location => $location,
          command  => "theme disable ${theme_name} --network",
          user     => $user,
          onlyif   => [
            "/bin/bash -c \"/usr/bin/wp theme list | /bin/grep -q ${theme_name}\"",
            "/bin/bash -c \"/usr/bin/wp theme list | /bin/grep ${theme_name} | /bin/awk '{print \\\$5}' | /bin/grep -q network\"",
          ],
          tag      => 'theme-uninstalled',
        }
        ->exec { "${location} sites disable theme ${theme_name}":
          command => "/bin/bash -c 'while read line; do /usr/bin/wp theme disable ${theme_name} --url=\$line; done <<< \"$(/usr/bin/wp site list --field=url)\"'",
          cwd     => $location,
          user    => $user,
          onlyif  => [
            "/bin/bash -c \"/usr/bin/wp theme list | /bin/grep -q ${theme_name}\"",
            "/bin/bash -c 'ret=0; while read line; do /usr/bin/wp theme list --url=\$line | /bin/grep ${theme_name} | /bin/awk \'{print \$5}\' | /bin/grep -q site; if [ $? -eq 0 ]; then let \"ret++\"; fi; done <<< \"$(/usr/bin/wp site list --field=url)\"; echo \$ret; /bin/test \$ret -gt 0'",
          ],
        ...
        ->exec { "${location} network deactivate theme ${theme_name}":
          ...
          onlyif  => [
            "/bin/bash -c \"/usr/bin/wp theme list | /bin/grep -q ${theme_name}\"",
            "/bin/bash -c 'ret=0; while read line; do if [ `/usr/bin/wp theme list | /bin/grep ${theme_name} | /bin/awk '{print \$2}'` == 'active' ]; then let \"ret++\"; fi; done <<< \"$(/usr/bin/wp site list --field=url)\"; /bin/test \$ret -gt 0'",
          ],
          before  => Wp::Command["${location} theme ${command}"],
          tag     => 'theme-uninstalled',
        }
