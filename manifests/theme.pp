  ...
  case $ensure {
    ...
    /^(absent|uninstalled)$/: {
      ...
      wp::command { "${location} disable theme ${theme_name}":
        ...
        user     => $user,
        onlyif   => "/bin/bash -c '/usr/bin/wp theme list --skip-plugins --skip-themes --skip-packages | /bin/grep ${theme_name} | /bin/awk \'{print \$5}\' | /bin/grep -q site'",
      ...
      if $networkwide {
        exec { "${location} network disable theme ${theme_name}":
          ...
          cwd     => $location,
          user    => $user,
          unless  => "/bin/bash -c 'ret=0; while read line; do /usr/bin/wp theme list --url=\$line --skip-plugins --skip-themes --skip-packages | /bin/grep -e ${theme_name} | /bin/grep -q \"network\|site\"; if [ $? -eq 0 ]; then let \"ret++\"; fi; done <<< \"$(/usr/bin/wp site list --field=url --skip-plugins --skip-themes --skip-packages)\"; echo \$ret; /bin/test \$ret == 0'",
        ...
        ->exec { "${location} network deactivate theme ${theme_name}":
          ...
          cwd     => $location,
          user    => $user,
          unless  => "/bin/bash -c 'ret=0; while read line; do /usr/bin/wp theme status ${theme_name} --url=\$line --skip-plugins --skip-themes --skip-packages | /bin/grep Status | /bin/grep -q Active; if [ $? -eq 0 ]; then let \"ret++\"; fi; done <<< \"$(/usr/bin/wp site list --field=url --skip-plugins --skip-themes --skip-packages)\"; /bin/test \$ret == 0'",
          before  => Wp::Command["${location} theme ${command}"],
          tag     => 'theme-uninstalled',
        }
      }
